...
def writeUserAction(request, question_id):
    ...
    if request.method == 'POST':
        ...
        if request.user.username == "":
            ...
            r = MockElectionUserVoteRecord(timestamp=timezone.now(),user=new_name,col=data,question=question,initial_order=init,final_order=final,device=device,initial_type=type,comment_time=commentTime,swit=swit,submit_time=submit_time,ui=UI)
            r.save()
        else:
            r = MockElectionUserVoteRecord(timestamp=timezone.now(),user=request.user.username,col=data,question=question,initial_order=init,final_order=final,device=device,initial_type=type,comment_time=commentTime,swit=swit,submit_time=submit_time,ui=UI)
            r.save()
        #f.close()
...
def downloadRecord(request, question_id):
    ...
    question = get_object_or_404(MockElectionQuestion,pk=question_id)
    records = question.mockelectionuservoterecord_set.all()
...
def downloadLatestVotes(request, question_id):
    ...
    question = get_object_or_404(MockElectionQuestion,pk=question_id)
    response_set = question.mockelectionresponse_set.filter(active=1).order_by('-timestamp')
    records = []
...
def downloadallocations(request):
    all_poll = list(MockElectionQuestion.objects.filter(question_type = 2).order_by('-pub_date'))
    # .question_set.all()
    result = []
...
def downloadPolls(request):
    all_poll = list(MockElectionQuestion.objects.filter(question_type = 1).order_by('-pub_date'))
    # .question_set.all()
    result = []
    for poll in all_poll:
        dic = {}
