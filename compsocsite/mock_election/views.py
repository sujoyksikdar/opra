from mock_election.models import *
...
class IndexView(views.generic.ListView):
    ...
    template_name = 'mock_election/index2.html'
...
class RegularPollsView(views.generic.ListView):
    ...
    template_name = 'mock_election/regular_polls.html'
    context_object_name = 'question_list'
...
class CodePollsView(views.generic.ListView):
    ...
    """
    template_name = 'mock_election/regular_polls_code.html'
    context_object_name = 'question_list'
...
class RegularPollsFolderView(views.generic.DetailView):
    ...
    template_name = 'mock_election/regular_polls_folder.html'
    model = Folder
...
class MainView(views.generic.ListView):
    ...
    template_name = 'mock_election/index.html'
    context_object_name = 'question_list'
...
class DemoView(views.generic.DetailView):
    ...
    def get_random_order(self, ctx) -> list:
        default_order = list(ctx['object'].mockelectionitem_set.all())
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.get_username() == "":
            ...
            if 'anonymousvoter' in self.request.session and 'anonymousid' in self.request.session:
                ...
                curr_anon_resps = self.object.mockelectionresponse_set.filter(anonymous_id=anon_id, active=1).reverse()
        ...
        currentUserResponses = self.object.mockelectionresponse_set.filter(user=self.request.user, active=1).reverse()
...
class CourseMatchView(views.generic.DetailView):
    ...
    def get_items_dict(self, ctx) -> dict:
        ...
        items_dict = {item.item_text.replace('\ufeff', ''): item for item in ctx['object'].mockelectionitem_set.all()}
        return items_dict
...
def addChoice(request, question_id):
    ...
    allChoices = question.mockelectionitem_set.all()
...
def deletePoll(request, question_id):
    ...
    if request.user != question.question_owner:
        return HttpResponseRedirect(reverse('mock_election:index'))
...
def pausePoll(request, question_id):
    ...
    if question.question_type == 1 and question.mockelectionresponse_set.filter(active=1).count() >= 1: #poll
...
def resumePoll(request, question_id):
    ...
    allItems = question.mockelectionitem_set.all()
...
def stopPoll(request, question_id):
    ...
    question.status = 3
    # get winner for regular polls only; allocation stop is handled by allocation:stopAllocation
...
def getCurrentSelection(mostRecentResponse):
    ...
    rd = responseDict
    array = []
    for itr in range(mostRecentResponse.question.mockelectionitem_set.all().count()):
        array.append([])
    for itr in rd:
        if rd[itr] != 1000:
...
class PollInfoView(views.generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        currentUserResponses = self.object.mockelectionresponse_set.filter(user=self.request.user,active=1).order_by('-timestamp')
        if len(currentUserResponses) > 0:
        ...
        all_responses = self.object.mockelectionresponse_set.filter(active=1).order_by('-timestamp')
        (latest_responses, previous_responses) = categorizeResponses(all_responses)
        ...
        # get deleted votes
        deleted_resps = self.object.mockelectionresponse_set.filter(active=0).order_by('-timestamp')
...
class VoteResultsView(views.generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        cand_map = getCandidateMapFromList(list(self.object.mockelectionitem_set.all()))
        ctx['cand_map'] = cand_map
        print(f"  - [get_context_data] cand_map: {cand_map}")

        response_count = list(self.object.mockelectionresponse_set.all())
...
def calculatePreviousResults(request, question_id):
    ...
    for pw in previous_winners:
        ...
        resultstr = ""
        movstr = ""
        responses = question.mockelectionresponse_set.reverse()
...
def show_polling_settings(request, question_id):
    ...
    return render(request, 'mock_election/_set_polling_settings.html', ctx)
...
def deleteUserVotes(request, response_id):
    ...
    if user: 
        question.mockelectionresponse_set.filter(user=user).update(active=0)
    else:
        question.mockelectionresponse_set.filter(anonymous_id=response.anonymous_id).update(active=0)
...
def restoreUserVotes(request, response_id):
    ...
    question = response.question
    if user: 
        question.mockelectionresponse_set.filter(user=user, active=0).update(active=1)
    else:
        question.mockelectionresponse_set.filter(anonymous_id=response.anonymous_id, active=0).update(active=1)
...
def getPrefOrder(orderStr, question):
    ...
    #if length != len(question.mockelectionitem_set.all()):
    #   return None
...
#     return HttpResponseRedirect(reverse('mock_election:coursematch', args=(question.id,)))
...
def anonymousVote(request, question_id):
    ...
    if 'anonymousid' not in request.session:
        ...
        id = question.mockelectionresponse_set.all().count() + 1
...
def mixtureAPI_test(request):
    context = RequestContext(request)
    return render(request, 'mock_election/api_test.html', context)
    ...
    if request.META.get('HTTP_X_REQUESTED_WITH') == 'XMLHttpRequest':
        ...
        for poll in polls:
            ...
            poll_json['curr_win'] = (poll.question_type == 1 and
                                    ...
                                    len(poll.mockelectionresponse_set.all()) > 0)
    ...
    if request.method == 'POST' and request.FILES.get('csvFile'):
        ...
        try:
            ...
            for row in csv_reader:
                ...
                # check duplicates
                if question.mockelectionitem_set.filter(item_text=item_name).exists():
                    items_skipped += 1
                    continue
...
def upload_bulk_images(request, question_id):
    ...
    if request.method == 'POST' and request.FILES.getlist('imageFiles'):
        try:
            ...
            all_items = question.mockelectionitem_set.all()
...
def delete_items(request, question_id):
    ...
    if request.method == 'POST':
        if 'delete_all' in request.POST:
            # delete all 
            question.mockelectionitem_set.all().delete()
            messages.success(request, "All items have been deleted.")
        else:
            # delete selected 
            try:
                item_ids = json.loads(request.POST.get('item_ids', '[]'))
                question.mockelectionitem_set.filter(id__in=item_ids).delete()
