...
def voteEmail(request, key, resp_id):
    ...
    if question.status == 2:
        arr = question.mockelectionitem_set.all().exclude(pk=eResp.item.pk)
...
class EmailThread(threading.Thread):
    def __init__(self, request, question_id, type, voters=None, mail_sub = None, mail_body = None):
        ...
        self.question = get_object_or_404(MockElectionQuestion, pk=question_id)
        self.type = type
    ...
    def run(self):
        ...
        for voter in self.voters:
            ...
            #         response.save()
            #         options += '<p><a href=\'' + self.request.build_absolute_uri(reverse('mock_election:index') + str(response.pk) + "/" + rand + "/voteEmail/") + '\'>' + i.item_text + '</a></p>'
            if voter.first_name != "":
